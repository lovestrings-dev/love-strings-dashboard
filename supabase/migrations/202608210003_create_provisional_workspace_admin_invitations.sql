-- A Platform Owner can stage a workspace without becoming a member of it.
-- Its first Admin is added only by the established ordinary invitation
-- acceptance RPC after the recipient completes authentication.

alter table public.app_workspaces
add column if not exists setup_state text not null default 'active';

alter table public.app_workspaces
drop constraint if exists app_workspaces_setup_state_check;

alter table public.app_workspaces
add constraint app_workspaces_setup_state_check
check (setup_state in ('pending_setup', 'active'));
...
begin
  ...
  -- Serialize retries/double clicks for one recipient without constraining
  -- ordinary invitations to existing live workspaces.
  ...
  values (
    staged_workspace_id,
    'Pending workspace',
    'pending-' || replace(staged_workspace_id::text, '-', ''),
    'pending_setup'
  );

  insert into public.app_workspace_settings (workspace_id)
  values (staged_workspace_id);
...
revoke all on function public.create_provisional_workspace_admin_invitation(uuid, text, text, timestamptz) from public;
grant execute on function public.create_provisional_workspace_admin_invitation(uuid, text, text, timestamptz) to service_role;
