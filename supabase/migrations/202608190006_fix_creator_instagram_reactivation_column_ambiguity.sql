-- The previous reconnect function correctly retained the named mapping unique
-- constraint, but its reactivation UPDATE used unqualified `connection_id`.
-- `connection_id` is also a RETURNS TABLE output variable, so PL/pgSQL rejects
-- that statement as ambiguous. Qualify the physical mapping column.
...
begin
  ...
  -- Disabled mappings remain history, but this same-account path explicitly
  -- reselects the canonical mapping after a disconnect.
  update public.app_meta_connection_accounts as mapping
    ...
    where mapping.workspace_id = p_workspace_id
      ...
      and mapping.account_type = 'instagram_professional'
