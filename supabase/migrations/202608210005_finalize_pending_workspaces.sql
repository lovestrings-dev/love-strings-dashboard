create or replace function public.finalize_pending_workspace(
  p_workspace_id uuid,
  p_user_id uuid,
  p_display_name text,
...
begin
  ...
  if not exists (
    ...
    where app_workspace_members.workspace_id = p_workspace_id and app_workspace_members.user_id = p_user_id and app_workspace_members.role = 'admin'
  ) then
...
revoke all on function public.finalize_pending_workspace(uuid, uuid, text, text) from public;
grant execute on function public.finalize_pending_workspace(uuid, uuid, text, text) to service_role;
