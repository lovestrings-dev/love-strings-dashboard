...
- Final acceptance polish: guided Google return now calculates a real desktop viewport-center position after the YouTube card settles. Mandatory first-Admin cadence/Distributor choices use a wider native ArtistDeck decision card with selected, focus, and disabled states. The canonical initial Auto Plan is shown as an explicit pale-orange 12-month planning horizon in both the General Roadmap and Plan 1; red remains reserved for a genuinely overdue scheduled release rather than an intentionally empty month.
