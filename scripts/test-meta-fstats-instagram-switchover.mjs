...
const facebookCollector = await readFile(new URL("../lib/metrics/meta-fstats-facebook-page-collector.ts", import.meta.url), "utf8");
...
assert.match(collectors, /name: "facebook", refresh: \(\) => refreshMetaFstatsFacebookPageMetrics\(workspaceId, createServiceSupabaseClient\(\)\)/);
assert.match(collectors, /hasEligibleMetaFstatsFacebookPageBinding\(workspaceId, supabase\)/);
assert.match(collectors, /Promise\.allSettled\(/, "Facebook failures remain isolated from the other shared collectors");
assert.match(facebookCollector, /\/me\/accounts[\s\S]*?access_token/);
