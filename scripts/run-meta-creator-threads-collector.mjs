...
const workspaceId = process.env.META_CREATOR_THREADS_TEST_WORKSPACE_ID ?? "00000000-0000-0000-0000-000000000001";
if (!process.argv.includes("--apply")) throw new Error("Refusing to write. Re-run with --apply for an intentional Threads collector test.");
...
const { refreshMetaCreatorThreadsMetrics } = await import(`data:text/javascript;base64,${Buffer.from(stripTypeScriptTypes(source, { mode: "strip" })).toString("base64")}`);
...
console.log(JSON.stringify(await refreshMetaCreatorThreadsMetrics(workspaceId, client), null, 2));
