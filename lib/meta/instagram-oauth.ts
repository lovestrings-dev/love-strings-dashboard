...
export async function exchangeCreatorSocialInstagramCode(code: string) {
  ...
  form.set("redirect_uri", getCreatorSocialInstagramRedirectUri());
  form.set("code", code);
  const payload = await readJson<TokenPayload>(await fetch("https://api.instagram.com/oauth/access_token", { method: "POST", body: form, cache: "no-store" }));
...
export async function fetchCreatorSocialInstagramIdentity(accessToken: string) {
