type InstagramAccount = { meta_external_id: string; account_name: string; url: string | null };
type InstagramMapping = {
  account_type: string;
  is_selected: boolean;
  platform_accounts: InstagramAccount;
};
type InstagramConnectionRow = {
  id: string;
  connection_state: string;
  token_expires_at: string | null;
  app_meta_connection_accounts?: InstagramMapping[];
};

export function resolveCreatorSocialInstagramState(rows: InstagramConnectionRow[]) {
  if (rows.length === 0) return { state: "disconnected" as const };
  const selected = rows
    .map((row) => ({
      row,
      mapping: row.app_meta_connection_accounts?.find((item) => item.account_type === "instagram_professional" && item.is_selected)
    }))
